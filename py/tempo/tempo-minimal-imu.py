...
imu.begin()
imu.setLowPassFilterFrequency("AccelLowPassFilter41")
...
publishName = b"imu"; #.encode('utf-8')
#--------end--------------
...
class Fetch(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            accData = [imu.AccelVals[0], imu.AccelVals[1], imu.AccelVals[2]]
            sensorData=[imu.AccelVals[0], imu.AccelVals[1], imu.AccelVals[2],imu.GyroVals[0], imu.GyroVals[1], imu.GyroVals[2],imu.roll, imu.pitch]
            #incoming(self, accData)
            #print (floats)
            publisherSocket.send_multipart([publishName, ",".join(map(str, sensorData)).encode('utf-8')])
            time.sleep(0.05)
